...
위의 모든 타입들의 오브젝트(object)는 literal을 사용해서 initialize할 수 있다. 예를 들어 'this is a string' 는 string literal 이고 true는 boolean literal이다.
...
int와 double 모두 num의 서브타입(subtype)이다. num 타입은 기본연산자인 +, -, /, * 과 abs(), ceil(), floor() 등 여러 method를 포함한다. (>> 같은 bitwise operator들은 int class에 정의되어 있다.) 만약 num이나 num의 서브타입인 int, double에 찾는 것이 없다면 dart:math 라이브러리에는 있을 것이다.
...
literal number는 컴파일 타임(compile-time) constant이다. 많은 산술 식들도 그 안에 연산자들(operands)이 컴파일 타임 constant이기 때문에 컴파일 타임 constant이다.
...
## Booleans
Boolean 값을 표현하기 위해 bool 타입을 사용한다. Bool 타입에는 두 개의 오브젝트가 있다. boolean literal인 true, false 이다. 둘 다 컴파일 타임 constant이다.
...
list는 zero-based indexing 을 사용한다. 첫번째 아이템의 index는 0이고 마지막 아이템의 인덱스(index)는 list.length – 1이 된다. list의 길이를 가져오는 방법과 list 값에 접근하는 방법은 자바스크립트와 같으니 익숙할 것이다.
...
list를 컴파일 타임 constant로 생성하기 위해서는 list literal 앞에 const를 붙인다.
...
Dart 2.3 은 또한 collection if 와 collection for을 제공한다. 이것을 사용하면 조건문(conditionals)과 반목문(repetition)을 사용하여 collection을 생성할 수 있다.

아래는 collection if를 사용하는 예시이다.
...
컴파일 타임 constant로 set을 만들기 위해서는 set literal 앞에 const를 추가한다. const 이기 때문에 새로 아이템을 추가하려고 하면 에러가 발생할 것이다.
...
Dart 2.3 부터 list 에서와 마찬가지로 set에서도 collection if, for 를 사용할 수 있다.
...
Note: 위의 코드에서 gifts는 Map<String, String> 타입을 가지고 nobleGases는 Map<int, String> 타입을 가진다. 만약 map에 틀린 타입의 값을 추가하려고 하면 에러가 발생할 것이다.
...
.length를 사용하면 key-value 쌍의 개수를 알 수 있다.
...
map literal 앞에 const를 붙이면 컴파일-타임 constant인 map을 생성할 수 있다.
{% highlight dart %}
...
유니코드는 전세계에서 사용되는 문자, 숫자, 기호에 대해 고유한 숫자 값을 정의하고 있다. Dart string은 UTF-16 코드 단위의 sequence이기 때문에 string 내에서 유니코드 부분을 구분하려면 특별한 syntax가 필요하다. 유니코드 부분을 구분하는 일반적인 방법은 \uXXXX이다. XXXX에서 X 하나가 16진수를 나타내므로 16진수가 총 4개 들어가는 것이다. 예를 들어 하트(♥)는 \u2665이다. 16진수를 4개보다 많거나 적게 사용하고자 하면 해당 값을 {} 괄호로 감싸야 한다. 예를 들어 웃는 이모티콘(😆)은 \n{1f606}이다.

만약 각각의 유니코드 문자를(character)를 읽거나 써야하는 경우에는 문자 패키지에서 문자열에 정의된 character getter를 사용하면 된다. return 된 Character 오브젝트는 grapheme clusters의 sequence 같은 string이다. 아래는 character API를 사용하는 예시이다.
