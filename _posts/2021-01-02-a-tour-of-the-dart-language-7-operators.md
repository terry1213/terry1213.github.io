---
title: A tour of the Dart language [7. Operators]
categories:
- Flutter
- Dart
tags:
...
obj가 T에 의해 명시된 인터페이스를 구현한다면 abj is T 의 결과는 true가 된다. 예를 들어 obj is Object는 항상 true이다.
...
만약 object가 T 타입인지 확신하지 못한다면, object를 사용하기 전에 is T를 통해 타입을 확인해라.
...
Note: 위의 두 코드는 같지 않다. 만약 emp가 null이거나 Person이 아니라면 첫번째 예시는 exception을 던지지만 두번째 예시는 아무것도 하지 않는다.
