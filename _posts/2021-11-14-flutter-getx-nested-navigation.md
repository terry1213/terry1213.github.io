...
이 글에선 GetX를 활용하여 중첩 네비게이션을 구현한다. 따라서 GetX의 네비게이션 부분에 대한 기본적인 이해가 필요하다. 만약 GetX에 대해서 알지 못한다면, [공식 패키지 사이트](https://pub.dev/packages/get){:target="\_blank"}나 [이전에 정리했던 글](https://terry1213.github.io/flutter/flutter-getx/){:target="\_blank"}을 읽어보고 오는 것을 추천한다.
...
예시에서는 `Get.toNamed()`만 사용했지만, GetX의 다른 네비게이션 기능에서도 다 적용된다.

또한 중첩 네비게이션이 꼭 `BottomNavigationBar`에만 적용할 수 있는 것이 아니다. 대표적으로 사용되는 상황인 것 뿐이다. 다른 위젯들도 화면 이동 시에 유지되도록 만들 수 있다.

따라서 자신의 상황에 맞춰서 적용하여 사용하면 되겠다.

## 주의할 점

중첩 네비게이션은 메모리에 스택처럼 아래부터 네비게이터를 쌓아가는 것이다. 이는 RAM 소비에 좋지 않으니 너무 과도하게 사용하지는 말자.
